...
from SG_pipeline.pipeline import find_swap_vq_code_info, insert_sg_2part

# endregion 


def generate(data_dir, audio_name, audio_path, gpt_path, rqvae_path, use_gpu, init_body_pose_code, init_hands_pose_code, local_rank, sg_codebook_path, retrieval_model_path):
    ...
    with torch.no_grad():
        ...
        # print("body_code_all: ", body_code_all.shape)
        # body_code_all = body_code_all.permute(1,0,2)
        # print("body_code_all: ", body_code_all.shape)
        ...
        # save_code = [body_code_all.cpu().numpy(), lower_code_all.cpu().numpy(), hands_code_all.cpu().numpy()]
        # np.save('test.npy', save_code)
    ...
    if need_insert_sg:
        # z_pred_body: [4, len]
        print('body_code_all', body_code_all.shape)
        z_pred_body = body_code_all.cpu().data.numpy()[:, 0]
        z_pred_hands = hands_code_all.cpu().data.numpy()[:, 0]
        # print(z_pred_body.shape)

        # gesture_rqcode_path = './SG_pipeline/part_zeroeggs_2part_root_in_body_sg.npz'
        gesture_rqcode_path = sg_codebook_path

        insert_sg_info_with_a_pose = find_swap_vq_code_info(audio_path, gesture_rqcode_path, retrieval_model_path, data_dir, 0)

        new_body_code, new_hand_code, save_insert_info = insert_sg_2part([z_pred_body, z_pred_hands], insert_sg_info_with_a_pose, data_dir, 0, sg_type='rq')
        
        insert_code_pos = []
        for insert_info in save_insert_info:
            s, e = insert_info['start_change_pos'], insert_info['end_change_pos']
            insert_code_pos.append([s, e])
        
        new_body_code_smooth = copy.deepcopy(new_body_code)
        new_hand_code_smooth = copy.deepcopy(new_hand_code)

        for insert_info in save_insert_info:
            s, e = insert_info['start_change_pos'], insert_info['end_change_pos']
            need_cal_pos = [s-3, s-2, s-1, e, e+1, e+2]
            need_pop_index = []
            for i in range(len(need_cal_pos)):
                if need_cal_pos[i] >= z_pred_body.shape[1]:
                    need_pop_index.append(i)
            for i in reversed(need_pop_index):
                need_cal_pos.pop(i) 

            need_cal_weight = torch.tensor([0.7, 0.5, 0.3, 0.3, 0.5, 0.7]).view(-1, 1).to(device)
            need_cal_weight = need_cal_weight[0:len(need_cal_pos)]
            need_cal_len = len(need_cal_pos)
            assert(len(need_cal_pos) == len(need_cal_weight))

            insert_sg_s_e_code = np.array(insert_info['insert_sg_s_e_code'])
            # input_body_sg: [4, 2]
            input_body_sg = torch.from_numpy(insert_sg_s_e_code[0]).to(device)
            sg_body = model_vqvae.module.vqvae_body.bottleneck.decode(input_body_sg)
            # sg_body: [2, 512]
            sg_body_s, sg_body_e = sg_body
            # sg_body: [2, 512] -> [6, 512]
            sg_body = torch.cat((sg_body_s, sg_body_s, sg_body_s, sg_body_e, sg_body_e, sg_body_e)).view(6, -1)[0:need_cal_len]

            input_hand_sg = torch.from_numpy(insert_sg_s_e_code[1]).to(device)
            sg_hand = model_vqvae.module.vqvae_hands.bottleneck.decode(input_hand_sg)
            sg_hand_s, sg_hand_e = sg_hand
            sg_hand = torch.cat((sg_hand_s, sg_hand_e, sg_hand_e, sg_hand_e, sg_hand_s, sg_hand_e)).view(6, -1)[0:need_cal_len]

            # input_body_raw: [4, need_cal_len]
            print(z_pred_body.shape, need_cal_pos)
            input_body_raw = torch.from_numpy(z_pred_body[:, need_cal_pos]).to(device)
            # raw_body: [need_cal_len, 512]
            raw_body = model_vqvae.module.vqvae_body.bottleneck.decode(input_body_raw)
            # cal_res_body: [6, 512]
            cal_res_body = (raw_body * need_cal_weight) + (sg_body * (1 - need_cal_weight))
            # cal_new_body_code: [4, 6]
            cal_new_body_code = model_vqvae.module.vqvae_body.bottleneck.encode(cal_res_body.unsqueeze(0).permute(0, 2, 1))
            cal_new_body_code = torch.stack(cal_new_body_code, dim=-1)[0].permute(1, 0).cpu().data.numpy()
            print(cal_new_body_code, z_pred_body[:, need_cal_pos], insert_sg_s_e_code[0])

            input_hand_raw = torch.from_numpy(z_pred_hands[:, need_cal_pos]).to(device)
            raw_hand = model_vqvae.module.vqvae_hands.bottleneck.decode(input_hand_raw)
            cal_res_hand = (raw_hand * need_cal_weight) + (sg_hand * (1 - need_cal_weight))
            cal_new_hand_code = model_vqvae.module.vqvae_hands.bottleneck.encode(cal_res_hand.unsqueeze(0).permute(0, 2, 1))
            cal_new_hand_code = torch.stack(cal_new_hand_code, dim=-1)[0].permute(1, 0).cpu().data.numpy()

            # cal_new_body_code: [4, len]
            new_body_code_smooth[:, need_cal_pos] = cal_new_body_code
            new_hand_code_smooth[:, need_cal_pos] = cal_new_hand_code

        new_body_code_smooth = torch.from_numpy(new_body_code_smooth).to(device).unsqueeze(0)
        new_hand_code_smooth = torch.from_numpy(new_hand_code_smooth).to(device).unsqueeze(0)
        tuple_code_smooth = ([new_body_code_smooth], [new_hand_code_smooth])

        generated_result_smooth_filter = model_vqvae.module.decode(tuple_code_smooth, insert_pos=insert_code_pos, filter_name='gaussian', filter_pos='motion_info')
        need_visulize_motion.append(generated_result_smooth_filter)
        save_name.append('semantic_results')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sg_codebook', type=str, required=True, default = '')
    parser.add_argument('--retrieval_model_path', type=str, required=True, default = '')
    ...
    generate(data_dir=new_save_dir, audio_name=audio_name, audio_path=args.audio_path, 
             ...
             sg_codebook_path = args.sg_codebook, retrieval_model_path = args.retrieval_model_path)