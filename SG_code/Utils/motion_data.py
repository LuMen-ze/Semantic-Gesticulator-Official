...
class MotionData:
    ...
    def to_facing_coordinate_for_first_frame(self):
        y_rot_res = MathHelper.y_decompose(self._joint_rotation[:, 0, :])[0][0]

        self._joint_translation[:, 0, :] = R(y_rot_res, copy=False, normalize=False).inv().apply(self._joint_translation[:, 0, :])
        
        rotation_after_y = (R(y_rot_res, copy=False, normalize=False).inv() * R(self._joint_rotation[:, 0, :], copy=False, normalize=False)).as_quat()
        rotation_after_y /= np.linalg.norm(rotation_after_y, axis=-1, keepdims=True)
        self._joint_rotation[:, 0, :] = rotation_after_y
