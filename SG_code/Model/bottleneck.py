...
class BottleneckBlock(nn.Module):
    ...
    def init_k(self, x):
        ...
        self.k_sum = self.k  # 把一个 batch 内映射到同一个 code 的所有 input 向量相加。
        self.k_elem = t.ones(k_bins, device=self.k.device)  # 计数每个 code 出现了几次。
    ...
    def update_k(self, x, x_l):
        ...
        with t.no_grad():
            ...
            # print(x_l_onehot.dtype, x.dtype)
            # assert(0)
            ...
            # usgae记录这个epoch中被使用过的code index，对应去更新这些code
            usage = (self.k_elem.view(k_bins, 1) >= self.threshold).float()  # k_bins, 1
            '''
                更新方法：
                    1. 如果某个code被使用过，那么就把选到它的几个新的input求平均作为新的code
                    2. 如果某个code未被使用过，那么就给他再重新随机生成一个
            '''
    ...
    # 返回的x_l是输入x后从codebook中找到的最近的那个code的下标
    def quantise(self, x):
    ...
    def one_hot_decode(self, one_hot):
        ...
        quantized_out = None
        b, l, d = one_hot.shape
        assert(d == self.k_bins)
        layer_quantized_out = t.matmul(one_hot.reshape(-1, self.k_bins), self.k)
        return layer_quantized_out.reshape(b, l, -1)
